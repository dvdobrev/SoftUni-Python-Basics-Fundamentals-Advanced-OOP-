class PizzaDelivery:
    ...
    def __init__(self, name: str, price: float, ingredients: dict):
        ...
        # self.ordered = False     # self.ordered should be instance argument, but not class argument. But Judge doesn't accept it like this!!!'
    ...
    def formated_order(self):
        return ', '.join([f"{k}: {v}" for k, v in self.ingredients.items()])

    def make_order(self):
        PizzaDelivery.ordered = True

        return f"You've ordered pizza {self.name} prepared with {self.formated_order()} and the price will be {self.price}lv."
