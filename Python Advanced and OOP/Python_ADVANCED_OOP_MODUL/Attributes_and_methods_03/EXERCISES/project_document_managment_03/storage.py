...
class Storage:
    ...
    def edit_category(self, category_id: int, new_name: str):
        searched_category = [c for c in self.categories if c.id == category_id][0]
        searched_category.name = new_name

    def edit_topic(self, topic_id: int, new_topic: str, new_storage_folder: str):
        searched_topic = [t for t in self.topics if t.id == topic_id][0]
        searched_topic.topic = new_topic
        searched_topic.storage_folder = new_storage_folder

    def edit_document(self, document_id: int, new_file_name: str):
        searched_document = [d for d in self.documents if d.id == document_id][0]
