from WORKSHO_08.Exercise_WORKSHOP import setup
from WORKSHO_08.Exercise_WORKSHOP.Helpers import positions_mapper
...
def is_diagonal_winner():
    diagonals = [setup.board[0][0] == setup.board[1][1] == setup.board[2][2] != setup.empty_spots,
                 setup.board[0][2] == setup.board[1][1] == setup.board[2][0] != setup.empty_spots]
    if any(diagonals):
        return True
    return False
...
def check_selected_position_is_valid(pos, counter):
    if not pos.isdigit() or not is_position_valid(pos):
        current_selected_position = input(
            f"{setup.player_names[counter % 2]}, select a position (number between 0-9): ")
